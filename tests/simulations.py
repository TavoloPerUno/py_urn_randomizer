...
def simulate_assignments(
    ...
    if fixed_participants:
        ...
        pdf_participants = pd.read_csv(
            os.path.join(simulation_data_folder, "participants.csv"),
            dtype=object,
            encoding="utf8",
        )
        pdf_participants = pdf_participants[
            ["id"]
            + ["f_" + factor for factor in sorted(study.factors.keys())]
            + ["user"]
        ]
        pdf_participants.to_csv(
            os.path.join(simulation_data_folder, "participants.csv"), index=False
        )
    for trial in range(n_simulations):
        process_isolated_trial(
            starting_seed,
            factor_combination_seed,
            trial,
            n_participants,
        ...
        factor_combination_seed = np.random.default_rng(
            factor_combination_seed
        ).integers(low=1, high=50000, size=1)[0]
        pdf_asgmt = pd.read_csv(
            os.path.join(simulation_data_folder, "trial_{0}.csv".format(trial)),
            dtype=object,
            encoding="utf8",
        )
...
def process_isolated_trial(
    trial_starting_seed,
    trial_factor_combination_seed,
    trial_no,
    n_participants,
    simulation_data_folder,
    study_name="Test Study",
    ...
    if fixed_participants:
        study.upload_new_participants(
            file=os.path.join(simulation_data_folder, "participants.csv")
        )
    else:
        study.generate_dummy_participants(n_participants, trial_factor_combination_seed)
    study.export_history(
        os.path.join(simulation_data_folder, "trial_{0}.csv".format(trial_no))
    )
    pdf_trial_data = pd.read_csv(
        os.path.join(simulation_data_folder, "trial_{0}.csv".format(trial_no)),
        dtype=object,
        encoding="utf8",
    )
    ...
    pdf_trial_data.to_csv(
        os.path.join(simulation_data_folder, "trial_{0}.csv".format(trial_no)),
        index=False,
    )


def simulate_assignments_mproc(
    n_participants,
    n_simulations,
    factor_combination_seed,
    starting_seed,
    ...
    lst_trial_seed = np.random.default_rng(starting_seed).integers(
        low=1, high=50000, size=n_simulations
    )
    lst_factor_combination_seed = np.random.default_rng(
        factor_combination_seed
    ).integers(low=1, high=50000, size=n_simulations)
...
def estimate_study_imbalance(simulation_label, study_name="Test Study"):
    ...
    lst_trial_files = [
        fname
        for fname in glob.glob(os.path.join(simulation_data_folder, "trial_*.csv"))
    ]
        ...
        for n_participants in range(1, max_participants + 1):
            ...
            study.upload_existing_history(
                pdf=pdf_trial_n[
                    [
                        col
                        for col in pdf_trial_n.columns
                        if col not in ["trial_no", "starting_seed"]
                    ]
                ]
                .iloc[[-1]]
                .copy()
            ...
            pdf_stats = (
                pdf_urns.copy()
                if pdf_stats is None
                else pd.concat([pdf_stats, pdf_urns], ignore_index=True).reset_index(
                    drop=True
                )
            )

    pdf_stats.to_csv(
        os.path.join(simulation_data_folder, "trial_stats.csv"), index=False
    )
    return pdf_stats
